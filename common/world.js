...
World.prototype.createObjects = function() {
	...
	for (var i = 0; i < numRugs; ++i) {
		...
		console.log(Math.floor(Math.random() * (room.width - rug.size.x)));
		rug.position = this.toTileCoord(new Vector2(Math.floor(Math.random() * (room.width - rug.size.x)),
			Math.floor(Math.random() * (room.height - rug.size.y))));
